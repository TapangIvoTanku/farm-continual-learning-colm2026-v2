...
All experiments were conducted using publicly available datasets (XSum, CNN/DailyMail, MedQA, GSM8K, HumanEval) and a publicly available base model (Mistral-7B-Instruct-v0.3, available on HuggingFace). Complete implementation details, including all hyperparameters, are provided in \S\ref{sec:experiments}. Source code, trained adapter weights, and full results --- including per-task performance matrices for all seven methods --- are provided in the anonymous supplementary repository at \url{https://anonymous.4open.science/r/farm-continual-learning-colm2026/}. All experiments use a fixed random seed (seed=42) and are fully deterministic given the same hardware. A single NVIDIA A100 80GB GPU is sufficient to reproduce all results; estimated total compute is approximately 80 GPU-hours across all methods.

% ─────────────────────────────────────────────────────────────────────────────
\section*{LLM Usage Statement}
AI-assisted writing tools were used during the preparation of this manuscript for grammar correction, sentence restructuring, and improving clarity of exposition. All scientific content, experimental design, results, and analysis are the work of the authors. No AI tool contributed to research ideation, experimental design, code implementation, or result interpretation.
